...
/**
 * Manhattan distance of two vector features.
 * 
 * @author Michal Ondrejcek
 * 
 */
public class ManhattanDistanceMeasure implements Measure {
	/**
	 ...
	 * @param feature2
	 *            another array
	 * @return the Manhattan distance between the two given arrays
	 * @throws Exception
	 */
	private SimilarityNumber compare(DoubleArrayFeature feature1,
		...
		double normalized = sum / (feature1.getLength() * distanceMax);
	...
	private SimilarityNumber compare(VectorFeature feature1, VectorFeature feature2) throws Exception {
		...
		double normalized = sum / (totalLength * distanceMax);
	...
	private SimilarityNumber compare(
		...
		for (int row = 0; row < feature1.getHeight(); row++) {
			for (int col = 0; col < feature1.getWidth(); col++) {
				for (int band = 0; band < feature1.getNumBands(); band++) {
					sum += Math.abs(feature1.getValue(row, col, band)
		...
		double normalized = sum
				/ (feature1.getHeight() + feature1.getWidth()
	...
	@Override
	public Class<ManhattanDistanceMeasure> getType() {
		return ManhattanDistanceMeasure.class;
	}
}